...
def _web_result():
    ...
    return ScanResult(
        ...
        findings=[],
        web_findings=[WebFinding(url="http://127.0.0.1/", category="security_header",
                                 detail="Missing CSP", severity="low", evidence="")],
        meta=WebMeta(url="http://127.0.0.1/", host="127.0.0.1",
                     resolved_ip="127.0.0.1", checks_run=19, checks_errored=0,
                     cve_findings=0, generated="2030-01-01T00:00:00+00:00"),
    )


def test_web_blocks_public_target_without_permission(monkeypatch):
    monkeypatch.setattr("blacklight.engine.guardrails.socket.getaddrinfo",
...
def test_web_end_to_end_private_target(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("blacklight.engine.WebScan.run",
                        lambda self, targets, params, on_progress=None: _web_result())
...
def test_web_exports_json(monkeypatch, tmp_path):
    monkeypatch.setattr("blacklight.engine.guardrails.socket.getaddrinfo",
                        lambda host, *a, **k: [("AF_INET", 1, 6, "", ("127.0.0.1", 0))])
    monkeypatch.setattr("blacklight.engine.WebScan.run",
                        lambda self, target, params, on_progress=None: _web_result())
    monkeypatch.setattr("blacklight.engine.paths.CACHE_DIR", tmp_path)
    monkeypatch.setattr("blacklight.engine.paths.SCAN_LOG", tmp_path / "scan.log")
...
def test_web_fails_gracefully_on_upstream_error(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("blacklight.engine.WebScan.run", boom)
    monkeypatch.setattr("blacklight.engine.paths.CACHE_DIR", tmp_path)
    monkeypatch.setattr("blacklight.engine.paths.SCAN_LOG", tmp_path / "scan.log")
    result = runner.invoke(app, ["web", "http://127.0.0.1"])
    assert result.exit_code == 1
...
def test_web_export_failure_reports_error(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("blacklight.engine.export_report", boom)
    result = runner.invoke(app, ["web", "http://127.0.0.1", "-o", str(out)])
    assert result.exit_code == 1
